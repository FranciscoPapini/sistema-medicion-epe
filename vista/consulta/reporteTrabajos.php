<?php
  ...
  $pdf = new PDF('P', 'mm', 'A4');
  $pdf->SetTitle(utf8_decode("Revisiones de Equipo"));
  $pdf->AliasNbPages();

// Equipo
if( count($_SESSION['reporteTrabajos']) > 0) { 

foreach ($_SESSION['reporteTrabajos'] as $consulta) {
...
if($consulta->getRespaldo() == ('on' || 1)) {
  ...
  if($consulta->getCabina() == '0') { $cabina = '-'; } else { if($consulta->getCabina() == '1') { $cabina = 'No Especificado'; } else { $cabina = $consulta->getCabina() . ' V'; } }
  $pdf->SetFont('Helvetica', 'B', 10);
...
//  $pdf->Cell(50, 10, '', 0, 1, '', 0);
  ...
  $pdf->SetX(35);
  $pdf->SetFont('Helvetica', '', 10);
  $pdf->Cell(105, 10, "No hay revisiones en la fecha seleccionada", 0, 0, 'R', 0);
